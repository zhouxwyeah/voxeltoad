...
│   ├── ingress/
│   │   ├── ingress.go   # Codec 接口 + 注册表（客户端入站协议翻译器，ADR-0045）
│   │   ├── openai/      # OpenAI 入站 codec（unified ≈ OpenAI，近乎 identity）
│   │   └── anthropic/   # Anthropic /v1/messages 入站 codec（Claude Code → OpenAI 上游）
│   ├── plugin/
...
| **新增一种客户端入站协议** | L2 | ① `internal/ingress/<name>/` 实现 `Codec` 接口（`DecodeRequest`/`EncodeResponse`/`NewStreamEncoder`/`EncodeError`）；② 在 `init()` 里 `ingress.Register`；③ `internal/proxy/router.go` 加路由 `r.Post(<path>, serveChat(ingress.Lookup(<protocol>), …))`；④ `codecForPath`（auth_middleware.go）加 path → protocol 映射；⑤ `internal/proxy/auth_middleware.go::bearerToken` 兼容该协议的鉴权头（如有）。无需碰 dispatcher/billing/telemetry（`serveChat` 协议无关）— 见 ADR-0045 |
